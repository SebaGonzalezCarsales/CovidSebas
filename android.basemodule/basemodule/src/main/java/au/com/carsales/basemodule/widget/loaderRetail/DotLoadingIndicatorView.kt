package au.com.carsales.basemodule.widget.loaderRetail
...
class DotLoadingIndicatorView : View {
    ...
    private var squareBorderColor: Int = 0xEEEEEE
    private var squareBorderStrokeWidthInDP: Int = 1
    ...
    private fun init() {
        ...
        backgroundBorderPaint?.color = squareBorderColor
        backgroundBorderPaint?.strokeWidth = dpToPx(squareBorderStrokeWidthInDP).toFloat(
     ...
     * setters
     */
    private fun setBackgroundBorder(color: Int? = 0xEEEEEE, widthDp: Int? = 1) {
        squareBorderColor = color ?: 0xEEEEEE
        squareBorderStrokeWidthInDP = widthDp ?: 1
    }
    ...
    fun setBackgroundVisibility(hasBackground: Boolean) {
        this.hasBackground = hasBackground
        reinitialize()
    }


    fun changeBackgroundBorder(color: Int? = 0xEEEEEE, widthDp: Int? = 1) {
        stopAnimation()
        setBackgroundBorder(color, widthDp)
        reinitialize()
    }
    ...
    @Deprecated("use fun showAsFullScreen(hasBackground: Boolean = false, hasFullScreenBackGround: Boolean = true) instead", ReplaceWith("show(null, hasBackground, false, hasBackground)"))
    fun showAsFullScreen(hasBackground: Boolean = false) {
        show(null, hasBackground, false, hasBackground)
    }

    fun showAsFullScreen(hasBackground: Boolean = false, hasFullScreenBackGround: Boolean = true) {
        show(null, hasBackground, false, hasFullScreenBackGround)
    }
    ...
    /*
            ...
            background: small square
            inverse: : for dot showing on top of button,
            we want to set the dot as white and set background same with the background of button

            hasFullScreenBackGround: //only applicable if anchorView is null
    * */
    @SuppressLint("ClickableViewAccessibility")
    fun show(
            anchorView: View?,
            background: Boolean = false,
            inverse: Boolean = false, //for dot showing on top of button,

            // we want to set the whole background same with the background of button
            hasFullScreenBackGround: Boolean = false //only applicable if anchorView is null
    ) {
        if (isShown) return
        ...
        context?.let {
            if (!(it as? Activity)?.isFinishing!!) {
                try {
                    wm.addView(parentView, windowLayoutParams)
                    anchorView?.addOnAttachStateChangeListener(object : OnAttachStateChangeListener {
    ...
    private fun drawBackground(canvas: Canvas) {
        ...
        backgroundPaint?.let {
            canvas.drawRoundRect(r, cornerRadius, cornerRadius, it)
        }

        //border
        backgroundBorderPaint?.let {
            canvas.drawRoundRect(r, cornerRadius, cornerRadius, it)
        }
    }
