...
/**
 * A FrameLayout with a rounded corner_sender background and shadow.
 * <p>
 ...
 * <p>
 * Due to expensive nature of rounded corner_sender clipping, on platforms before Lollipop, CardView does
 ...
 */
public class CSCardView extends ConstraintLayout {
    ...
    public CSCardView(@NonNull Context context) {
        this(context, null);
    }

    public CSCardView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.cardViewStyle);
    }

    public CSCardView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray customTypedArray = context.obtainStyledAttributes(attrs, R.styleable.csn_cardView, defStyleAttr,
                                                                     R.style.CardView);
        int cornerFlag = 0;
        boolean flag = customTypedArray.getBoolean(R.styleable.csn_cardView_csn_roundCardLeftTopCorner, true);
        cornerFlag += flag ? CSRoundRectDrawable.FLAG_LEFT_TOP_CORNER : 0;

        flag = customTypedArray.getBoolean(R.styleable.csn_cardView_csn_roundCardRightTopCorner, true);
        cornerFlag += flag ? CSRoundRectDrawable.FLAG_RIGHT_TOP_CORNER : 0;

        flag = customTypedArray.getBoolean(R.styleable.csn_cardView_csn_roundCardLeftBottomCorner, true);
        cornerFlag += flag ? CSRoundRectDrawable.FLAG_LEFT_BOTTOM_CORNER : 0;

        flag = customTypedArray.getBoolean(R.styleable.csn_cardView_csn_roundCardRightBottomCorner, true);
        cornerFlag += flag ? CSRoundRectDrawable.FLAG_RIGHT_BOTTOM_CORNER : 0;

        customTypedArray.recycle();

        IMPL.initialize(mCardViewDelegate, context, backgroundColor, radius,
                        elevation, maxElevation, cornerFlag);
    }
    ...
    /**
     * Updates the corner_sender radius of the CardView.
     *
    ...
    /**
     * Returns the corner_sender radius of the CardView.
     *
    ...
    private final CardViewDelegate
        ...
        @Override
        public boolean getUseCompatPadding() {
            return CSCardView.this.getUseCompatPadding();
        }

        @Override
        public boolean getPreventCornerOverlap() {
            return CSCardView.this.getPreventCornerOverlap();
        }

        @Override
        public void setShadowPadding(int left, int top, int right, int bottom) {
            mShadowBounds.set(left, top, right, bottom);
            CSCardView.super.setPadding(left + mContentPadding.left, top + mContentPadding.top,
        ...
        public void setMinWidthHeightInternal(int width, int height) {
            if (width > mUserSetMinWidth) {
                CSCardView.super.setMinimumWidth(width);
            }
            if (height > mUserSetMinHeight) {
                CSCardView.super.setMinimumHeight(height);
            }
        }
    ...
    /**
     * show corner_sender or rect
     */
    public void showCorner(boolean leftTop, boolean rightTop, boolean leftBottom, boolean rightBottom) {
        if (SDK_LOLLIPOP) {
            ((CSRoundRectDrawable) getBackground()).showCorner(leftTop, rightTop, leftBottom, rightBottom);
        } else {
